...
| --- | --- | --- | --- | --- |
| `/day [天数]` | 查询运营数据统计 | 默认 `0`（当天），`1` 为昨日，`2-30` 为最近 `N` 天，超过 `30` 按 `30` 处理 | - | 用户新增/到期/续费、订单金额与笔数、支付渠道汇总、总收入 |
...
- **查询范围限制**：为避免大范围查询带来的压力，命令查询窗口最大为最近 30 天。

## 实现说明

- 订单、用户和支付渠道统计均优先在数据库侧完成聚合，避免把整批已支付订单读入内存。
- 当支付渠道为空时，仍会单独汇总手动操作订单，避免收入统计缺失。
